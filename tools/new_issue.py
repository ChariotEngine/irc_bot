...
def detect_bugs(self, e_bugs):
    ...
    for i in range(0, 16):
        if remote_bugs[i] not in e_bugs:   # it's a new bug
            ...
            self.send_message_to_channel(("New %s #%s by %s: %s | https://github.com/ChariotEngine/Chariot/%s/%s")
                                        % (report_type,
                                            str(remote_bugs[i]),
                                            y[i]['user']['login'],
                                            y[i]['title'],
...
def bugs_list(self):
    ...
    while True:
        try:
            ...
            print("*** [%s] Could not fetch a list of bugs, apparently 'Exceed Rate Limit'" % self.irc_host)
            time.sleep(7200)    # wait 2 hours

def isFirstPR(self, reportedBy):
    url = 'https://api.github.com/search/issues?q=repo:ChariotEngine/Chariot+type:pr+author:%s' % reportedBy
    try:
        data = urllib.request.urlopen(url).read().decode()
        e = json.loads(data)
        if e['total_count'] == 1:
